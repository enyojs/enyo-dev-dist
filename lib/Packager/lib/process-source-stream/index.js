...
function ProcessSourceStream (opts) {
	...
	if (opts.paths.length) {
		opts.paths = opts.paths.map(function (search) {
			return search.charAt(0) == '/' ? search : path.resolve(opts.cwd, search);
		});
	}
...
ProcessSourceStream.prototype._transform = function (entry, nil, next) {
	...
	if (entry.resolved) this._modules[entry.name] = entry;
...
ProcessSourceStream.prototype._flush = function (done) {
	...
	queue.forEach(function (entry) {
		...
		if (entry.resolved) {
			log.info({module: entry.relName}, 'cached entry found, not resolving');
			stream._resolving++;
...
ProcessSourceStream.prototype._resolve = function (entry) {
	...
	if (modules[entry.name]) {
		log.debug({module: entry.relName}, 'already resolved, skipping');
		return done();
	}
	...
	resolver(entry, opts, this._libs, this._resolved);
};
...
ProcessSourceStream.prototype._resolved = function (err, entry) {
	...
	if (entry.contents) {
		...
		if (!entry.resolved) {
			...
			try {
				...
				return utils.streamError(stream, 'failed to parse file %s', entry.fullpath);
			}
			
			log.info({module: entry.relName}, 'parsing module contents for expandable URI paths');
			
			try {
				uriResolver(entry, stream.options);
			} catch (e) {
				return utils.streamError(stream, 'failed to parse file %s for expandable URI paths', entry.fullpath);
			}
		}
	}
	...
	if (entry.requires.length || entry.requests.length) {
		...
		entry.requires.forEach(function (dep) {
			entry.dependencies.push(stream._dependency(entry, dep));
		});
		entry.requests.forEach(function (dep) {
			entry.dependencies.push(stream._dependency(entry, dep, true));
		});
	}
...
ProcessSourceStream.prototype._finalize = function () {
	...
	for (i = 0; i < skipped.length; ++i) {
		...
		if (entries && entries.length) {
			...
			for (j = 0; j < entries.length; ++j) {
				...
				if (idx === -1) {
					utils.streamError(this, 'unable to locate dependency entry for skipped module %s from dependent module %s', skip.relName, mod.relName);
				}
	...
	if (errored.length) utils.streamError(this, 'cannot continue due to encountered errors');
